package cn.com.yibin.maomi.core.redis;
...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	public    static final int           MAX_RETRY_SENTINEL = Integer.MAX_VALUE;
    ...
    protected AtomicBoolean 			 reInitedPool       = new AtomicBoolean(false);
    ...
    public ShardedJedisSentinelPool(List<String> masters, Set<String> sentinels,
		...
		List<HostAndPort> masterList = initSentinels();
		initPool(masterList);
    }
...
//		}
    ...
    private void initPool(List<HostAndPort> masters) {
    	if (!equals(currentHostMasters, masters)) {
    		...
    		LogUtil.error(getClass(),"Created ShardedJedisPool to master at [" + sb.toString() + "]");
	...
	public  void  reInitPool(){
		if(reInitedPool.compareAndSet(false, true)){
			try{
				List<HostAndPort> masterList = initSentinels();
				initPool(masterList);
			}finally{
				reInitedPool.set(false);
			}
		}
	}
	@SuppressWarnings("resource")
	public  List<HostAndPort>  initSentinels() {
    	Map<String, HostAndPort> masterMap = new HashMap<String, HostAndPort>();
    	List<HostAndPort> shardMasters = new ArrayList<HostAndPort>();
    	
    	LogUtil.error(getClass(),"Trying to find all master from available Sentinels...");
	    ...
	    for (String masterName : masters) {
					...
					try {
					    ...
					    if (master == null) {
					    	...
					    	if (hostAndPort != null && hostAndPort.size() > 0) {
					    		LogUtil.error(getClass(),"Connecting to Sentinel " + hap +"=====Connecting to redis " + hostAndPort);
						...
						e.printStackTrace();
						LogUtil.error(getClass(),"Cannot connect to sentinel running @ " + hap + ". Trying next one.");
					}
		    	}
		    	
		    	if (null == master) {
		    		try {
						LogUtil.error(getClass(),"All sentinels down, cannot determine where is "
	    	...
	    	if (!fetched && sentinelRetry >= MAX_RETRY_SENTINEL) {
	    		LogUtil.error(getClass(),"All sentinels down and try " + MAX_RETRY_SENTINEL + " times, Abort.");
...
//    private   boolean checkJedisIsMaster(Jedis shard){
//    	return (-1 < shard.info("Replication").indexOf("role:master")) ;
//    } 
