...
//            // 获取xml所在包
//            MapperScannerConfigurer config = context.getBean
...
//        }
        ...
        public void run() {
            try {
                if (scanner.isChanged()) {
                    LogUtil.warn(this.getClass(),"*Mapper.xml文件改变,重新加载.");
                    scanner.reloadXML();
                    LogUtil.warn(this.getClass(),"加载完毕.");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    class Scanner {
        ...
        public Resource[] getResource(String basePackage, String pattern) throws
                IOException {
        ...
        private void clearMap(Class<?> classConfig, Configuration configuration, String
        ...
        private void clearSet(Class<?> classConfig, Configuration configuration, String
                fieldName) throws Exception {
            Field field = classConfig.getDeclaredField(fieldName);
            field.setAccessible(true);
